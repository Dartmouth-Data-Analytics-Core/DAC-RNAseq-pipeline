...
# The QC verdict section lives in its own stdlib-only module so that
# scripts/add_qc_report.py can reuse it without pulling in numpy and pandas.
...
def render_html(payload, title, subtitle, footer, logo="", assessment=""):
    ...
    out = out.replace("__ASSESSMENT__", assessment)
...
def main(argv=None):
    ...
    ap.add_argument("--qc-report", default=None,
                    help="Markdown (or HTML) QC write-up to render as a section at the "
                         "bottom of the page. Default: <run-dir>/qc_assessment.md when "
                         "present. Pass an empty string to omit it.")
    ...
    report_path = find_qc_report(run_dir, args.qc_report)
    assessment = build_assessment(report_path)
    if report_path:
        log(f"QC report section from {report_path}")

    out_path = os.path.abspath(args.output)
    os.makedirs(os.path.dirname(out_path) or ".", exist_ok=True)
    with open(out_path, "w", encoding="utf-8") as fh:
        logo = "" if args.logo == "" else embed_logo(args.logo)
        fh.write(render_html(sanitize(payload), args.title, subtitle, footer, logo, assessment))
